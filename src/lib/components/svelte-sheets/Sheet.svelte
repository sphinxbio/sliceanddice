<script lang="ts">
  ...
  function getRowHeight(i: number) {
    try {
      const height = Number(
        typeof rows[i]?.height == "string"
          ? rows[i]?.height?.replace("px", "")
          : rows[i]?.height || config.defaultRowHeight // Use config.defaultRowHeight here
      );

      return height > config.defaultRowHeight ? height : config.defaultRowHeight; // Use config.defaultRowHeight here
    } catch (e) {
      return config.defaultRowHeight; // Use config.defaultRowHeight here
    }
  }
  ...
  hotkeys("ctrl+c, command+c, ctrl+x, command+x", function (e) {
    ...
    const textToCopy = selectedData?.map(row => row.join('\t'))?.join('\n');

    navigator.clipboard.writeText(textToCopy).then(function() {
      console.log('Selected data copied to clipboard');
    }, function(err) {
      console.error('Could not copy selected data: ', err);
    });
  });

  function getSelectedData(data, selected) {
    if(!selected) return;
    
    const [start, end] = selected.map(cell => decode(cell));
...
>
  ...
  >
    ...
    >
      ...
      />
      <colgroup>
        <col width={config.defaultHeaderColWidth} />
        {#each visibleX as v}
