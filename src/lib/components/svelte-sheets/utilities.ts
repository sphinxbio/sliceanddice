...
function getRowHeight(row, options) {
  try {
    const height = Number(
      typeof row?.height == "string"
        ? row?.height?.replace("px", "")
        : row?.height || options.defaultRowHeight // consider adding a config.defaultRowHeight
    );
    return height > options.defaultRowHeight ? height : options.defaultRowHeight;
  } catch (e) {
...
export const computeStyles = (c, r, row, style, options, value, nextValue) => {
  ...
  height: ${getRowHeight(row, options)}px;
